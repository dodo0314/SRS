...
//   strand: 'uw'(기본) | 'en' 영어 코스 몫 | 'stats' 통계 코스 몫 | 'srs' 카드 창(코스의 카드 정거장이 대체) | 'info' 배너
...
/* ---------- P1 주차 진도표 (W1=8/31~) ----------
   ...
   sunB: 일요일 블록 덮어쓰기
   satB: 토요일 학습 B의 ICC 해부 일정 — 2026-09-06부터 학습 B는 시뮬레이터 세션이라(하루설계 ⊕ 결정)
         지금은 쓰지 않는다. ICC 해부를 어느 창에 다시 놓을지는 v2/v3 통합과 함께 일요일에 정한다 */
const P1_WEEKS = [
...
const T = {
  ...
  // 토요일 학습 B(14:00~17:00). 하루설계 ⊕ 2026-09-06 결정 — 재물손사 시험 포기로 답안 작성 블록이
  // 시뮬레이터 세션이 됐다. 형식은 74번(STR 룰 검증 수렴 — 시뮬레이터를 채점 기계로 재정의한 문서) 6-2절.
  sim: {
    strand: 'stats',
    stage: '정리',
    key: 'sim',
    title: '시뮬레이터 세션 — STR 룰 채점 3h',
    dur: '3h',
    detail:
      '① 5분 인출: 코드 안 보고 지난주 그래프가 왜 그렇게 나왔는지 말로 ② 10분 오늘의 질문 하나 — 돌리기 전에 예측을 숫자로(알림 건수·탐지율) ' +
      '③ Claude가 배관을 짜는 동안 다음 룰을 한국어 한 문장으로 ④ 파라미터는 직접 넣고 직접 돌린다 ⑤ 예측 vs 결과 — 틀린 것이 오늘 배울 것. ' +
      '내 해석을 먼저 쓰고 반례를 시킨다 ⑥ 15분 이해 노트 한 줄 + 카드 3~5장 → 통계/탐지평가 덱(세션 끝에만, 미리 만들지 않는다). ' +
      '회사 임계값·룰 로직·알림 통계는 어떤 형태로도 반입 금지. 산출물은 코드·그래프·이해노트·카드뿐 — 분석 문서 금지.',
  },
  wfhRule: {
    strand: 'info',
    title: '⚠ 재택일 규칙',
...
function saturday(now, phase) {
  return [
    ...
    { name: '학습 B 14:00~17:00 — 산출 전용', tasks: [{ id: 's2', ...T.sim }] },
    {
